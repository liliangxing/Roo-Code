...
import {
	getLockGuardedToolExecutor,
	LockGuardedToolExecutor,
	type LockAcquisitionResult,
} from "../../services/file-lock"
...
export async function presentAssistantMessage(cline: Task) {
	...
	switch (block.type) {
		...
		case "tool_use": {
			...
			// Phase 7b: Acquire file locks for write tools before execution.
			// This prevents concurrent tasks from writing to the same file simultaneously.
			// Lock acquisition is only attempted for complete (non-partial) blocks with valid args.
			let lockResult: LockAcquisitionResult | undefined
			if (!block.partial && block.nativeArgs) {
				const executor = getLockGuardedToolExecutor()
				lockResult = executor.tryAcquireLocks(
					block.name as ToolName,
				...
				if (!lockResult.success) {
					const errorMsg = LockGuardedToolExecutor.formatLockConflictError(lockResult.conflicts)
			...
			try {
				...
				// Phase 7b: Release file locks acquired before tool execution.
				if (lockResult?.success && lockResult.lockedPaths.length > 0) {
					getLockGuardedToolExecutor().releaseLocks(lockResult.lockedPaths, cline.taskId)
				}
			}
